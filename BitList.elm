...
fromNumberWithLength : Int -> Int -> List(Bit)
fromNumberWithLength size number =
  let
...
fromByte byte = fromNumberWithLength 8 byte
...
partition size list =
  if ((List.length list)<=size) then
    ...
    (List.take size list) :: (partition size (List.drop size list))
